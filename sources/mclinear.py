# Copyright 2022 xymb@endcrystal.me
...
def quickly_verify_linear(file_path):
    ...
    signature, version, newest_timestamp, compression_level, chunk_count, complete_region_length, hash64 = struct.unpack_from(
        ">QBQbhIQ", raw_region, 0)
    ...
    for i in range(REGION_DIMENSION * REGION_DIMENSION):
        ...
        if region.chunks[i] != None:
            ...
            if sector_count > 255:
                ...
                chunk_file_path = destination_folder + "/c.%d.%d.mcc" % (
                region.region_x * 32 + x, region.region_z * 32 + z)
...
def write_region_anvil_to_bytes(region: Region,
                                compression_level=zlib.Z_DEFAULT_COMPRESSION):  # CAREFUL: Doesn't support MCC!
    SECTOR = 4096
...
def write_region_linear_v2(destination_filename, region: Region, compression_level=1, grid_size=8):
    ...
    for i in range(32 ** 2):
        ...
        if region.chunks[i] is not None:
            chunks[x][z] = struct.pack(">IQ", len(region.chunks[i].raw_chunk) + 8, region.timestamps[i]) + \
                           region.chunks[i].raw_chunk
    ...
    preheader = struct.pack(">QBQbii", LINEAR_SIGNATURE, LINEAR_VERSION, newest_timestamp, grid_size, region.region_x,
                            region.region_z)
    ...
    for x in range(grid_size):
        for z in range(grid_size):
            ...
            if bucket_size > 0:
                ...
                if bucket_hash != hash.digest(): raise Exception("Bucket hash incorrect! Bitrot?",
                                                                 len(compressed_bucket))
                iterator = 0
                for ix in range(32 // grid_size):
                    for iz in range(32 // grid_size):
                        ...
                        if True:
                            ...
                            chunks[chunk_index] = Chunk(chunk_data, REGION_DIMENSION * region_x + chunk_index % 32,
                                                        REGION_DIMENSION * region_z + chunk_index // 32)
